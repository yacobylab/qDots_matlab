...
% model: echo signal: y0_i+a*cos(w_i * x + phi_i) exp(-(x/x0_i)^2) exp(-(tau/t2)^alpha)
% p(1)=t2
% p(2)=alpha
% p(3)=a
% p(4+5i)=w_i
...
opts=def(opts,'opts','residuals colorplot plotdbz');
...
%% Load the data.
s=ana_avg(fname,struct('grps',opts.grps));
...
if ~isempty(opts.dbz)
    for i=1:length(opts.dbz)
        ...
        fp=fitwrap('fine',s.xv{opts.dbz(i)},dbzdata',fifn, cosfn2, [1 1 1 1 0 1]);
...
for j=1:length(s.grps)
    ...
    mmod(j).fn=@echo_fit;
    off=5*(j-1);
    mmod(j).pt=@(p) [ p(1) p(2) p(3) s.tv(j)  p(4+off) p(5+off) p(6+off) p(7+off) p(8+off)];
    ...
    initial=[initial, p(4), atan2(p(2),p(3)), 40,mean(mdata(j).y), 0];
end
...
for j=1:length(s.grps)
    mask=initial*0;
    mask(5+5*(j-1))=1;
    out.p=mfitwrap(mdata(j),mmod(j),out.p,'plinit plfit plotiter err',mask);
end
mask=initial*0+1;
mask(2)=0;
[out.p out.chisq, out.cov]=mfitwrap(mdata,mmod,out.p,'plinit plfit plotiter err',mask);
[out.p out.chisq, out.cov]=mfitwrap(mdata,mmod,out.p,'plinit plfit plotiter err');
...
y=p(8)+p(3)*cos(p(5)*x+p(6)) .* exp(-abs((x-p(9))/p(7)).^2 - abs(p(4)/p(1))^p(2));
end
