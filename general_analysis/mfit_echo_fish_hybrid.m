function [out figures]=mfit_echo_fish_hybrid(fname,opts)
% Simultaneous constrained fit on echo data.
% hybrid 1/f + white noise model.
% model: echo signal: y0_i+a*cos(w_i * x + phi_i) exp(-((x-x0_i)/t2*)^2) exp(-(tau/t2) - (tau/t2f).^2)
...
opts=def(opts,'fixflicker',nan);
opts=def(opts,'fixwhite',nan);
fignum=1024;
figures=[];
...
%% Load the data.
s=ana_avg(fname,struct('grps',opts.grps,'opts','noplot noppt'));
...
if isopt(opts,'fishless')
    initial=[1/max(s.tv) (1/max(s.tv))^2 1 0 30];
else
  initial=[1/max(s.tv) (1/max(s.tv))^2 1 0.5 30];
end
if ~isnan(opts.fixflicker)
    initial(2)=opts.fixflicker;
end
if ~isnan(opts.fixwhite)
    initial(1)=opts.fixwhite;
end
...
y=p(10)+p(3)*cos(p(7)*x+p(8)) .* exp(-abs((x-p(11))/p(5)).^2 - abs(p(6)*p(1)) - abs(p(2)*p(6)*p(6))) + ...
