function [data, scalefuncs, mv,fp]=anaHistScaleGrpwise(scan, data,t1s,grpsw)
...
for i=1:length(t1s)
   ...
   n=squeeze(sum(sum(data{nds+i+1}(:,grps,:),2),1));
   n(end)=[];
   %figure(400+i);
   %clf; hold on;
   fp=fitwrap('',v,n',initfn,fitfn,[1 1 1 1 0 0 1]);    
   ...
   data{i}(:,grps,:) = scalefuncs{i}(data{i}(:,grps,:));   
    end
end
