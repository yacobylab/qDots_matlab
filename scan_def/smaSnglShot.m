...
ind = [find(ic(:, 1) ~= ic(1, 1)); find(ic(:, 1) == ic(1, 1))]; % move those channels last
scan.loops(loop).getchan = scan.loops(loop).getchan(ind);
ic = ic(ind, :);
scan.configfn(1).args{2} = find(ind == scan.configfn(1).args{2});
...
if strfind(ctrl, 'dec')   %
    ...
    if 0 % mask already configured in configfn
        readout = round(readout(1, 2:3) * samplerate(2)*plsdata.tbase/awgdata.clk);
        % convert to AWG samples.
        zl = plsinfo('zl', plsgrp.name);
        n =  samplerate(2) * abs(zl(1, 1))/awgdata.clk; %#hw samples per pulse

        smdata.inst(ic(end, 1)).data.mask = false(1, n);
        smdata.inst(ic(end, 1)).data.mask(readout(1)+1:readout(1)+readout(2)) = true;
        scan.configfn(1).args{3}(2) = samplerate(2)/n; %passed to smabufconfig
    end
