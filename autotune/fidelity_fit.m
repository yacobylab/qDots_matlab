function [fidelityFit tmeas vthresh t1] = fidelity_fit(ctrl, scan,data, bins,drop_time)
%function [fidelityFit tmeas vthresh t1] = fidelity_fit_fun(ctrl, scan,data,drop_time,bins)
%bins is set to 512 if not given. 
% drop_time is set to 120*20 ns if not given. 
...
[SfidFit TfidFit] = FidelityFit(hist',Vt,t1,dt,singHist',tripHist',samp_num,timestep,short);   %the fit function takes in raw data, the set of voltage bins, t1, averaging time, 
...
if ~isempty(strfind(ctrl,'fpl'))
    figure(81); clf;
    figure(82); clf;
    for i=1:4
        t=8+(i-1)^2*35;
        plotter(hist',Vt ,t1, dt,singHist',tripHist',samp_num,t,i);
    end
end
if ~isempty(strfind(ctrl,'pl'))
    figure(83); clf;
...
plot(dt*short*(1:timestep),VtFit/bins)
xlabel('Time')
ylabel('V_{threshold}')
...
subplot(2,2,1); hold on;
plot(fidUnf);
title(sprintf('Fidelity = %.2f  T_{meas} = %.2f \\musec\n V_{threshold} = %.1f mV ', fidelityUnf, 1e6*dt*TmeasUnf,1e3*Vt(VtUnf(TmeasUnf))));
subplot(2,2,2); 
plot(fidFit(:,6:end))
title(sprintf('Fidelity = %.2f  T_{meas} = %.2f \\musec\n V_{threshold} = %.1f mV ', fidelityFit, 1e6*dt*TmeasFit*short,1e3*Vt(VtFit(TmeasFit))));

subplot(2,2,3); hold on
plot(1e6*T,maxfidUnf,1e6*T2,maxfidFit);
xlabel('T (\mus)');
title('Best Fidelity');
 
fprintf('Fidelity = %.3f (unfit) vs. %.3f (fit) \n Tmeas = %.2f usec (unfit) vs. %.2f usec (fit) \n Vthreshold = %3f (unfit) vs. %3f (fit) \n', fidelityUnf, fidelityFit, dt*TmeasUnf*1e6, dt*short*TmeasFit*1e6, 1e3*Vt(VtUnf(TmeasUnf)), 1e3*Vt(VtFit(TmeasFit)));
...
function [SfidFit TfidFit] = FidelityFit(hist,Vt ,t1, dt,singHist,tripHist, samp_num,timestep,short)
...
function plotter(hist,Vt ,t1, dt,singHist,tripHist,samp_num,t,i)
    [fitfn, initfn] = getfn(t*dt*1e-4,t*dt/t1);
