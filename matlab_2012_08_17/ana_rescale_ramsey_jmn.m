function out = ana_rescale_ramsey_jmn(file, config)
...
for j = 1:length(file)
    ...
    if isfield(scan, 'data') && isfield(scan.data,'FPGA') && ~isempty(scan.data.FPGA)
    ...
    if isopt(config,'fpga') %Use FPGA data in place of Fourier data.
        ...
        %results(j).fourier_data(:,1:end) = results(j).fpga_data<config.fpga_thresh;
...
% results(j).fourier_data=results(j).evo_data;
% config.ftime=results(j).t;
...
for j = 1:length(results)
    if isopt(config,'bayes')        
        ...
        %config = def(config,'bayes_freq',[.001,.02]);
        bayes_freq=config.bayes_freq; 
        if bayes_freq(1)>0.3 
            warning('This is a large frequency. Please input frequency in GHz')
        end
        [dbz stddbz] = get_dbzbayes_act(results2(j).fourier_data,config.ftime,bayes_freq,config.nfreqs,calib,config.threshold);                
    ...
    elseif isopt(config, 'fpgafreqs') %load the freqs from the FPGA and use them for the correction
        ...
        dbz = get_dbzthreshold(results2(j).fourier_data,config.ftime, .010);
        dbzall=[dbzall; dbz]; 
        if isfield(config,'fourier_inds2') && ~isempty(config.fourier_inds2)
...
%Sweep through detunings, and reconstruct for all data sets separately and together. Store all of the
...
dend=.0675;
dpoints=25;
detunings=linspace(dstart,dend,dpoints);
detunings(end+1)=.065;
detunings=0;
...
for k=1:length(detunings);
    ...
    %results(j).dbzs_ramsey=abs(results(j).dbzs-detunings(k));
    ...
    %Take mean ramsey dbz across all data sets.
    %mdbz=mean(mean([results(j).dbzs_ramsey]));
...
out.results=results; 
out.config = config;
out.detunings=detunings;
offset=round(.075*size(out.newtime(1,:),2));
...
if isopt(config,'multi') && ~isopt(config, 'nofit')
    ...
    for j = 1:length(data)
        plot(data(j).x,out.results(j).newdata(ind,mask2),'x');
...
elseif ~isopt(config,'nofit')
    ...
    pars = [.45, .3, 116, 500, 1.6,pi/2];
    lb = [.1 .1 75 50 1  0];
    ub=  [.5 .5 500 4000 2 2*pi];
...
%Need minus sign here to account for overall pi pulse from beginning
%ramsey.
fdata1=-1*(2*(fdata>thresh)-1); %size nreps x nt
