function out = ana_rescale_dbz(file, config)
%function out = ana_rescale_dbz(file, config)
...
%filename{end+1}='sm_ramsey_CDS_R_6479.mat'
if  ~exist('config','var')
    config = struct();
end
...
evos = xv; evos(shots) = []; % these are the evolution times
nevo = length(evos);
...
if isopt(config,'digital')
   data(:,shots) = data(:,shots)>0.5; 
end

if isopt(config,'nobayes')
   scale = mean(data(:,shots),2);
elseif isopt(config,'dmm')
    scale=data{3};
else
   scale= findscale(data); 
end
S_av = mean(scale);
scale = scale-S_av;
oldscale = scale;
...
function bestbeta = findscale(data)
