...
%(discuss this)
ts=(-1)^s*range(datah2)/1.6; %slope for tanh function, given range tanh about 1.6 over center
Th=1/(8.617e-5*2*.231);
mp=find(~logical(datah2-min(datah2))) %minplace 
sl=(datah2(end)-min(datah2))/(epsilonval(end)-epsilonval(mp))

beta0 = [0 ts Th sl 1e-5 5e-6 .154];
fits = fitwrap('plinit plfit', epsilonval1, datah2, beta0, @qpcTraceFit2, logical([1 1 0 1 1 1 1])) ;
...
beta0 = [0 ts 8.9e4 sl 1e-5 5e-6 lev];
fits2 = fitwrap('plinit plfit', epsilonval2, datac2, beta0, @qpcTraceFit2, logical([1 1 1 1 1 1 0]));
Tnew = 1/(2*8.617e-5*fits2(3))
Tc = fits2(6)
