...
if isopt(config, 'fidfit')
    ...
    % <X> = S-T axis
    % New parameters: initial amplitude, initial phase, axis error left, axis error right, entanglement rate        
    pfunc=@paulivec;
    
   
    [m mi]=max(bestfids);
    cguess=[.9 0 0,0,pi/(2*xvals(mi))];
    fmtstring='';
    fitdescr='';
...
ct = cos((x+ip)*went);
st = sin((x+ip)*went);
pv = ia * [ o * sin(phi1), ct * cos(phi1), cos(phi1)*st*sin(phi2), ... % XI,YI,ZI
            o * sin(phi2), ct * cos(phi2), cos(phi2)*st*sin(phi1), ... % IX,IY,IZ
            ct * cos(phi2) * sin(phi1), ...                            % XY
            cos(phi2)*st,...                                           % XZ
            ct * cos(phi1) * sin(phi2),...                             % YX
            0,...                                                      % YZ
            cos(phi1) * st,...                                         % ZX
            0,...                                                      % ZY
            sin(phi1) * sin(phi2),...                                  % XX
            cos(phi1) * cos(phi2),...                                  % YY
            0];                                                        % ZZ
end
