...
config = def(config, 'opts','fid norm fidfit sqwd cal ana sqn');   % Random boolean options
...
if isopt(config,'norm')
    ...
    if isopt(config,'err')
       errorbar(xvals,norms, sqrt(norms_variances),'k');       
    end
end
...
if isopt(config, 'fidfit')
    ...
    % <X> = S-T axis
    pfunc=@(p,x) [cos((x(:)+p(2))*p(3)) * p(1), ...  % <YI> - like coefficent
        cos((x(:)+p(2))*p(3)) * p(1), ...  % <IY> - like coefficent
        repmat(p(1)^2,length(x),1),   ...  % <XZ> - like coefficent
        sin((x(:)+p(2))*p(3)) * p(1), ... % <ZX> - like coefficent
        sin((x(:)+p(2))*p(3)) * p(1)]; % <YY> - like coefficent
    [m mi]=max(bestfids);
    ...
    if isopt(config,'tqwd') % two-qubit gate white-noise dephasing
        n=length(cguess);
        pfunc=@(p,x) pfunc(p,x) .* [exp(-(x(:)/p(n+1)).^1),exp(-(x(:)/p(n+1)).^1), ones(length(x),1), ...
...
if isopt(config,'mineigen')  % Check physicality of density matrix
  figure(200+figind); 
  figind=figind+1;
  c=[];
  mi=[];
