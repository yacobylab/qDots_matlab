function grpdef = plsmakegrp_parallel(name, pulsedata, AWGdata,ctrl, ind, opts)
...
for k = 1:length(name)
    if(~isstruct(name{k}))
       ...
       plslog=[];
       s=load([plsdata.grpdir, 'pg_', name{k}]);
       plslog = s.plslog;
       grpdef = s.grpdef;
    ...
    switch grpdef.ctrl(1:min([end find(grpdef.ctrl == ' ', 1)-1]))
        ...
        case 'pls'
            ...
            for m = 1:length(ind)
                ...
                grpdef.pulses(m) = plstowf_parallel(plsdef(m), [],plsdata);
