...
function qdsaveppt(filespec,text,prnopt)
  ...
  g1=[];
    for i=1:length(text.consts)
        if strcmp(text.consts(i).setchan,'Time')
                g1{i}=[text.consts(i).setchan sprintf('\t%s ','=') datestr(text.consts(i).val,'yy-mm-dd HH:MM:SS')];            
        else
            if text.consts(i).val < 1e6
                g1{i}=[text.consts(i).setchan sprintf('\t%s ','=') num2str(text.consts(i).val)];
            else
                g1{i}=[text.consts(i).setchan sprintf('\t%s ','=') sprintf('%4e',text.consts(i).val)];
            end
        end
    end
   g=sprintf('%s\n',g1{:});
...
text2 = invoke(new_slide.Shapes(1),'AddTextbox',1,single(5),single(5),single(maxwidth),single(100));
set(text2.TextFrame.TextRange.Font,'Size',single(10));
set(text2.TextFrame.TextRange,'Text',datestr(now,'yy-mm-dd HH:MM:SS'))

if isfield(text,'file') && ~isempty(text.file)
textf = invoke(new_slide.Shapes(1),'AddTextbox',1,slide_W-5-maxwidth,single(5),maxwidth,single(100));
set(textf.TextFrame.TextRange.Font,'Size',single(10));
set(textf.TextFrame.TextRange,'Text',text.file)    
end


% Make a textbox for body text.
text1 = invoke(new_slide.Shapes(1),'AddTextbox',1,single(180),single(108+get(pic1,'Height')),single(maxwidth),single(175));
set(text1.TextFrame.TextRange.Font,'Size',single(10));
set(text1.TextFrame.TextRange,'Text',sprintf(g2));
