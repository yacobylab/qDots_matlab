...
for k = 1:length(name)
    ...
    switch grpdef.ctrl(1:min([end find(grpdef.ctrl == ' ', 1)-1]))
        ...
        case 'pls'
            if isempty(strfind(grpdef.ctrl,'multi'))
                if isfield(grpdef,'varpar') && size(grpdef.varpar,1)==1 && size(grpdef.varpar,2)>1
                ...
                for m = 1:length(ind)
                    ...
                    if iscell(grpdef.varpar)
                        grpdef.pulses(m).xval = [grpdef.varpar{i}(j, end:-1:1), grpdef.pulses(m).xval];
                ...
                if ~iscell(grpdef.varpar)
                   grpdef.varpar = {grpdef.varpar}; 
                end
                if ~iscell(grpdef.params)
                   grpdef.params = {grpdef.params}; 
                end
                if length(grpdef.pulses)>1
                   if length(grpdef.varpar)==1
                      vp = grpdef.varpar{1};
                       grpdef.varpar = cell(1,length(grpdef.pulses));
                       for jj = 1:length(grpdef.varpar)
                          grpdef.varpar{jj}= vp; 
                       end
                   end
                   if length(grpdef.params)==1
                      pp = grpdef.params{1};
                      grpdef.params = vell(1,length(grpdef.pulses));
                      for jj = 1:length(grpdef.params)
                         grpdef.params{jj} = pp; 
                      end
                   end
                end
                if length(grpdef.varpar)~=length(grpdef.params)||length(grpdef.varpar)~=length(grpdef.pulses)
                   error('trying to use ''multi'' option with inconsistent dimensions'); 
                end
                lens = cell2mat(cellfun(@(x)length(x),grpdef.varpar,'UniformOutput',0)); %length of each varpar
                npar = max(1,sum(lens));
                pinds = ones(1,lens(1));%this will be lookup table to find which varpar and params to use
                for j = 2:length(lens)
                ...
                plsdef = grpdef.pulses(pinds(ind));%(ind);
...
%                 end
                ...
                for m = 1:length(ind)
                    ...
                    if ~isempty(grpdef.varpar{p})
                        grpdef.pulses(m).xval = [grpdef.varpar{p}(j, end:-1:1), grpdef.pulses(m).xval];
                    end
                end
            end
    ...
    switch ctrl(1:min([end find(ctrl == ' ', 1)-1]))
        ...
        case 'upload'
            if ~isempty(strfind(ctrl, 'force')) || plsinfo('stale',grpdef.name)
                ...
                if isfield(grpdef.pulses(1).data,'readout')
                  ...
                  for ll=1:length(grpdef.pulses)
                    if ~isempty(grpdef.pulses(ll).data(1).readout)
                      ...
                      plens(ll) = size(grpdef.pulses(ll).data.marker,2);
                    end
                  end
                  chg=find(squeeze(any(abs(diff(readout,[],3))>1e-4)));
                  if isempty(chg)
                     rd.readout = readout(:,:,1);
                     rd.reps = size(readout,3);
                     rd.plens = size(grpdef.pulses(1).data.marker,2);
                  else
                      chg = chg';
                     rd.readout = readout(:,:,[1, 1+chg]);
                     rd.reps = [chg(1),diff(chg),size(readout,3)-chg(end)];
                     rd.plens = plens([1 1+chg]);
                  end
                  if any(abs(diff(readout,[],3)) > 1e-10)
                    warning('Readout changes between pulses in %s\n',name{k});
                  end
                  if(size(readout,1) > 0)
                    plslog(end).readout = rd;%readout(:,:,1);
                  else 
                    plslog(end).readout=[];
                  end
                end
                ...
                for i=1:length(grpdef.pulses) 
                    l=length(grpdef.pulses(i).xval(:));
                    plslog(end).xval(1:l,i)=[grpdef.pulses(i).xval(:)];
                end
%                plslog(end).xval = vertcat(grpdef.pulses.xval)'; % temporary bug fix
                plslog(end).ind = ind;
                
                %save([plsdata.grpdir, 'pg_', name{k}], '-append','-v6', 'plslog', 'zerolen');
                save([plsdata.grpdir, 'pg_', name{k}], '-append','-v7', 'plslog', 'zerolen'); %2012/12/26
