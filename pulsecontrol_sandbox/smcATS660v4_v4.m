function [val, rate] = smcATS660v4_v4(ico, val, rate, varargin)
% val = smcATS660v2(ico, val, rate, varargin)
% This version of the DAQ driver supports groups that change the
% pulselength inside the group. It references
% smdata.inst(x).data.num_pls_in_grp
                    ...
                    if ~isempty(s.subs{1})
                        if length(s.subs{1})==downsamp %old style mask
                            ...
                            if isfield(smdata.inst(ico(1)).data,'num_pls_in_grp')...
                                    ...
                                    && ~isnan(smdata.inst(ico(1)).data.num_pls_in_grp)
                                npls = smdata.inst(ico(1)).data.num_pls_in_grp;
                                nloop = nsamp/npls;
...
%                pause(.1);
            ...
            case 7 % the num_pls_in_grp flag
