function polarizefn(x, ctrl, pulses, workp, duration, bfield)
% polarizefn(x, ctrl, pulses, workp, duration)
...
switch length(workp)
    case {3, 6}
        smset({'GateR', 'GateL', 'GateQPC'}, workp(1:3));
    case {2, 4}
    ...
    case 9
        if dopol
            ...
            smset({'GateR', 'GateL', 'GateQPC'}, workp(7:9));
        end
...
switch length(workp)
    case 4
        smset({'GateR', 'GateL'}, workp(3:4));
    case {6 9}
        smset({'GateR', 'GateL', 'GateQPC'}, workp(4:6));
end
