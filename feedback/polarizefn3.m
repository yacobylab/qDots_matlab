function polarizefn3(x, loop, pulsetab, workp, gates)
...
% mode == 9: use sequence line pulsetab(i, 3) + 2 * fbdata.pulseind
...
for i = pulses;
    ...
    switch pulsetab(i, 6)
        ...
        case 9
            if ~isempty(fbdata.pulseind)
                pulsetab(i, 3) = pulsetab(i, 3) + 1 * fbdata.pulseind;
                if length(fbdata.fbx) >= 1*fbdata.pulseind+2
                    fbdata.xval=fbdata.fbx(1*fbdata.pulseind+2);
                end
                % jumps to first pulse in sequence for untriggered sequence
                % and pulsetab(i, 3) = awgseqind(sequence#).
            else
                pulsetab(i, 3) = 0;
            end
    end
    
    if any(pulsetab(i, 6) == 6:9)
        if pulsetab(i, 4) > 0
    ...
    if pulsetab(i, 5) > 0
        smset(gates, workp(pulsetab(i, 5), :));
    end
    ...
    if abs(sweep) > 0 && pulsetab(i, 5) > 0 % sweep gates while pulse running
